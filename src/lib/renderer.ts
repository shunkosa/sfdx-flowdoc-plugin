...
export default class Renderer {
        ...
        for(let i=0; i<decisions.length; i++ ) {
            ...
            if (decisions[i].rules.connector) {
                ...
                if (lastAction.connector) {
                    const scheduledActionSections = this.flowParser.getScheduledActionSections(lastAction.connector.targetReference);
    ...
    renderDecision(decision, criteria) {
        ...
        if (criteria === 'FORMULA_EVALUATES_TO_TRUE') {
            ...
            table.table.body.push([ this.th(this.i18n.__('CONDITION_LOGIC')), decision.rules.conditionLogic.toUpperCase() ])
        }
        return table;
    }
